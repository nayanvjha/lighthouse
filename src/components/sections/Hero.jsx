...
function Hero({ ready = true, reducedMotion = false }) {
  ...
  useEffect(() => {
    ...
    if (reducedMotion) {
      ...
      gsap.set(blackScreenRef.current, { autoAlpha: 0 })
      return undefined
    }
  ...
  return (
    <section
      id="hero"
      className="relative flex min-h-screen items-center justify-center overflow-hidden px-4 pb-16 pt-24 md:px-10"
      style={{
        background:
    ...
    >
      ...
      >
        ...
        <h1 className="mt-5 flex flex-wrap justify-center gap-x-3 gap-y-1 font-heading text-[2.5rem] leading-[1.05] tracking-[0.08em] text-white drop-shadow-[0_0_16px_rgba(0,163,255,0.45)] md:text-[5rem]">
        ...
        <p
          ...
          className="mt-5 max-w-2xl text-[1.1rem] leading-relaxed text-gray-300 md:text-[1.15rem]"
        ...
        <div
          ref={actionsRef}
          className="mt-8 flex w-full max-w-md flex-col gap-3 pointer-events-auto sm:flex-row sm:justify-center"
        >
          <HudButton variant="blue" onClick={() => scrollTo('projects')} className="justify-center">
            EXPLORE MISSIONS
          </HudButton>
          <HudButton variant="green" onClick={() => scrollTo('contact')} className="justify-center">
            INITIATE CONTACT
          </HudButton>
        </div>
      </div>
